let products = [
  {
    name: 'agate1',
    image: '/images/agate1.jpg',
    shape: 'pear',
    size: '8.2 carat',
    price: '$22.75',
    inStock: '100',
    text: 'Agate is formed in the cavities of igneous rocks, through deposits of silica which settle into the characteristic banded patterns we see in all forms of this semi-precious stone. It measures 7 on the Mohs scale of hardness.',
    __v: 0,
  },
  {
    name: 'aquamarine1',
    image: '/images/aquamarine1.jpg',
    shape: 'pear',
    size: '894.26.84carat',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'aquamarine2',
    image: '/images/aquamarine2.jpg',
    shape: 'pear',
    size: '120.00 carat',
    price: '$670.00',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'aquamarine5',
    image: '/images/aquamarine5.jpg',
    shape: 'pear',
    size: '64.50 carat',
    price: '$722.00',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'aquamarine6',
    image: '/images/aquamarine6.jpg',
    shape: 'pear',
    size: '25.28 carat',
    price: '$158.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'agate7',
    image: '/images/agate7.jpg',
    shape: 'pear',
    size: '6.2 carat',
    price: '$52.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'aquamarine7',
    image: '/images/aquamarine7.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Aquamarine is the green-blue to blue variety of the mineral beryl. (Emerald is the green to bluish green variety of the same mineral. Its color is usually a light pastel greenish blue.',
    __v: 0,
  },
  {
    name: 'citrine1',
    image: '/images/citrine1.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'agate2',
    image: '/images/agate2.jpg',
    shape: 'pear',
    size: '6.6 carat',
    price: '$19.55',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'citrine2',
    image: '/images/citrine2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'citrine3',
    image: '/images/citrine3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'agate3',
    image: '/images/agate3.jpg',
    shape: 'pear',
    size: '9.5 carat',
    price: '$47.50',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'citrine4',
    image: '/images/citrine4.jpg',
    shape: 'pear',
    size: '7.8mm x 4.9mm',
    price: '$82.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'agate4',
    image: '/images/agate4.jpg',
    shape: 'pear',
    size: '12.9 carat',
    price: '$64.75',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'citrine5',
    image: '/images/citrine5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Its pale yellow color is what gives it its name, Citrine, coming from the French citron, meaning lemon. Citrine has a deep spiritual meaning, too. The spiritual meaning of Citrine is its yellow hues symbolizing the spiritual qualities of joy and \nabundance.',
    __v: 0,
  },
  {
    name: 'citrine6',
    image: '/images/citrine6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'citrine7',
    image: '/images/citrine7.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'agate5',
    image: '/images/agate5.jpg',
    shape: 'pear',
    size: '7.0 carat',
    price: '$20.99',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'aquamarine3',
    image: '/images/aquamarine3.jpg',
    shape: 'pear',
    size: '84.50 carat',
    price: '$476.50',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'agate6',
    image: '/images/agate6.jpg',
    shape: 'pear',
    size: '3.9 carat',
    price: '$12.99',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'diamond2',
    image: '/images/diamond2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'aquamarine4',
    image: '/images/aquamarine4.jpg',
    shape: 'pear',
    size: '82.50 carat',
    price: '$652.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amazonite1',
    image: '/images/amazonite1.jpg',
    shape: 'pear',
    size: '20.0 carat',
    price: '$62.50',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'diamond3',
    image: '/images/diamond3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'diamond4',
    image: '/images/diamond4.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amazonite2',
    image: '/images/amazonite2.jpg',
    shape: 'pear',
    size: '15.0 carat',
    price: '$55.50',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'diamond5',
    image: '/images/diamond5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Diamond is a solid form of the element carbon with its atoms arranged in a crystal structure called diamond cubic. Diamond has the highest hardness and thermal conductivity of any natural material, properties that are used in major industrial applications such as cutting and polishing tools. ',
    __v: 0,
  },
  {
    name: 'diamond6',
    image: '/images/diamond6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amazonite3',
    image: '/images/amazonite3.jpg',
    shape: 'pear',
    size: '6.2 carat',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'emerald1',
    image: '/images/emerald1.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'emerald2',
    image: '/images/emerald2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amazonite4',
    image: '/images/amazonite4.jpg',
    shape: 'pear',
    size: '26.84 carat',
    price: '$82.99',
    inStock: '100',
    text: 'Also called "Amazon stone, amazonite is the only microcline you are likely to encounter as a gemstone. Microclines are potassium feldspars with a triclinic crystal structure Amazonite color can range from pale green to dark green and blue-green.',
    __v: 0,
  },
  {
    name: 'emerald3',
    image: '/images/emerald3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amazonite5',
    image: '/images/amazonite5.jpg',
    shape: 'pear',
    size: '42.24 carat',
    price: '$110.00',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'emerald4',
    image: '/images/emerald4.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'emerald5',
    image: '/images/emerald5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'emerald6',
    image: '/images/emerald6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Emerald is a life-affirming stone and one of infinite patience. Emerald is the stone of love and friendship and keeps partnerships in balance. This gem also opens up to your latent abilities and promotes the gathering of wisdom from the mental planes.',
    __v: 0,
  },
  {
    name: 'amazonite6',
    image: '/images/amazonite6.jpg',
    shape: 'pear',
    size: '64.20 carat',
    price: '$168.99',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'emerald7',
    image: '/images/emerald7.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal1',
    image: '/images/opal1.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amazonite7',
    image: '/images/amazonite7.jpg',
    shape: 'pear',
    size: '18.50 carat',
    price: '$210.35',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal2',
    image: '/images/opal2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst1',
    image: '/images/amethyst1.jpg',
    shape: 'pear',
    size: '894.26.84carat',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal3',
    image: '/images/opal3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'diamond1',
    image: '/images/diamond1.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst2',
    image: '/images/amethyst2.jpg',
    shape: 'pear',
    size: '69.69 carat',
    price: '$189.99',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal4',
    image: '/images/opal4.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal5',
    image: '/images/opal5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal6',
    image: '/images/opal6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst3',
    image: '/images/amethyst3.jpg',
    shape: 'pear',
    size: '72.84 carat',
    price: '$172.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'opal7',
    image: '/images/opal7.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Opal is the product of seasonal rains that drenched dry ground in regions such as Australia’s semi-desert “outback.” The showers soaked deep into ancient underground rock, carrying dissolved silica (a compound of silicon and oxygen downward.',
    __v: 0,
  },
  {
    name: 'ruby1',
    image: '/images/ruby1.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst4',
    image: '/images/amethyst4.jpg',
    shape: 'pear',
    size: '28.24 carat',
    price: '$115.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'ruby2',
    image: '/images/ruby2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'ruby3',
    image: '/images/ruby3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst5',
    image: '/images/amethyst5.jpg',
    shape: 'pear',
    size: '100.58 carat',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'ruby4',
    image: '/images/ruby4.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst6',
    image: '/images/amethyst6.jpg',
    shape: 'pear',
    size: '894.26.84carat',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'ruby5',
    image: '/images/ruby5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'ruby6',
    image: '/images/ruby6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Rubies are a form of the mineral corundum. The deep red color of a good quality Ruby is heartwarming and uplifting to look upon. You can feel the vigor of the energies of this remarkable stone as soon as you touch it or have it near you.',
    __v: 0,
  },
  {
    name: 'amethyst7',
    image: '/images/amethyst7.jpg',
    shape: 'pear',
    size: '894.26.84carat',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'ruby7',
    image: '/images/ruby7.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'sapphire1',
    image: '/images/sapphire1.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'amethyst8',
    image: '/images/amethyst8.jpg',
    shape: 'oval',
    size: '6.2mm x 5.2mm',
    price: '$104.15',
    inStock: '100',
    text: 'Amethyst is one of the most famous crystals because of its beautiful coloration as well as its spiritual and literal meaning.The spiritual meaning of Amethyst is groundedness, tranquility, and calm. The meaning of Amethyst is “not intoxicated” – its name translates from the Greek word améthystos.',
    __v: 0,
  },
  {
    name: 'sapphire2',
    image: '/images/sapphire2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'sapphire3',
    image: '/images/sapphire3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Sapphire is known as a wisdom stone, with each color carrying its own special wisdom. Sapphire calms the mind and helps you to focus. It also deals with unwanted thoughts and is a good choice for anyone seeking to deal with over-thinking or over-analyzing. ',
    __v: 0,
  },
  {
    name: 'sapphire4',
    image: '/images/sapphire4.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'sapphire5',
    image: '/images/sapphire5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'sapphire6',
    image: '/images/sapphire6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'sapphire7',
    image: '/images/sapphire7.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'zircon1',
    image: '/images/zircon1.jpg',
    shape: 'round',
    size: '6.8mm x 5.9mm',
    price: '$48.28',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'zircon2',
    image: '/images/zircon2.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: 'Colorless zircon is well known for its brilliance and flashes of multicolored light, called fire. These two zircon properties are close enough to the properties of diamond to account for centuries of confusion between the two gems.',
    __v: 0,
  },
  {
    name: 'zircon3',
    image: '/images/zircon3.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'zircon4',
    image: '/images/zircon4.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'zircon5',
    image: '/images/zircon5.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
  {
    name: 'zircon6',
    image: '/images/zircon6.jpg',
    shape: 'pear',
    size: '6.2mm x 3.9mm',
    price: '$42.15',
    inStock: '100',
    text: '',
    __v: 0,
  },
];
module.exports = products;
